...
// // ====== 목업 모드 설정 ======
// const USE_MOCK_DATA = true; // 실제 API 사용시 false로 변경
...
//     return MOCK_DATA.accountInfo;
//   }
//   // 실제 API 호출
//   // const userData = await getMe();
//   // return { hasAccount: !!userData.data.account, accountNumber: userData.data.account };
//   return { hasAccount: false };
// };
...
//   // 실제 API 호출
//   // const storesData = await getStores();
//   // return storesData.data.stores.map(store => ({ id: store.id, name: store.name }));
//   return [];
// };
...
//     return { ...MOCK_DATA.workSession, storeId };
//   }
//   // 실제 API 호출
//   return {
//     storeId,
...
//   if (USE_MOCK_DATA) {
//     await new Promise(resolve => setTimeout(resolve, 500));
//     console.log('목업: 출근 처리 완료', storeId);
//     return;
//   }
//   // 실제 API 호출
//   // await createMyTimesheet(storeId);
...
//   if (USE_MOCK_DATA) {
//     await new Promise(resolve => setTimeout(resolve, 500));
//     console.log('목업: 퇴근 처리 완료', storeId, timesheetId);
//     return;
//   }
//   // 실제 API 호출
//   // await patchMyTimesheetCheckout(storeId, timesheetId);
...
//     if (visible && storeId) {
//       // 목업 데이터 로드
//       const info = MOCK_DATA.storeDetails[storeId as keyof typeof MOCK_DATA.storeDetails];
//       setDetailInfo(info);
//       setEditedInfo(info);
//     }
//   }, [visible, storeId]);
...
//   isCheckIn: boolean;
// }) => {
//   const [nfcStatus, setNfcStatus] = useState<'waiting' | 'checking' | 'success'>('waiting');
//   const [currentTime, setCurrentTime] = useState('');
//   const slideAnim = useRef(new Animated.Value(SCREEN_HEIGHT)).current;
//   const scaleAnim = useRef(new Animated.Value(1)).current;
...
//     switch (nfcStatus) {
//       case 'waiting': return `아직 ${isCheckIn ? '출근' : '퇴근'}하지 않았어요`;
//       case 'checking': return 'NFC 태그를 확인 중...';
//       case 'success': return `${isCheckIn ? '출근' : '퇴근'} 완료!`;
//       default: return '';
//     }
//   };
